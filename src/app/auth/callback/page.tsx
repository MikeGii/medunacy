  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data?.session?.user) {
          ...
          if (access_token && refresh_token) {
            console.log('Found tokens in URL, attempting manual session setup...');
            ...
            try {
              ...
              if (!sessionStorage.getItem('callback_attempted')) {
                ...
                setErrorMessage('Failed to verify email. Please try clicking the verification link again.');
                setStatus('error');
              }
            }
          } else {
            console.log('No tokens found in URL');
            setErrorMessage('No authentication data found in the verification link');
            setStatus('error');
          }
        }
      } catch (error) {
        console.error('Callback error:', error);
        setErrorMessage(`Verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        setStatus('error');
      }
    };
    ...
    const handleSuccessfulAuth = async (user: any) => {
      try {
        ...
        setErrorMessage('Authentication succeeded but verification update failed');
        setStatus('error');
      }
    };
      ...
      <div className="max-w-sm w-full bg-white rounded-2xl shadow-lg p-12 text-center">
        {status === 'loading' && (
          <div>
            ...
            <p className="text-sm text-gray-600 mt-2">Verifying your account...</p>
          </div>
        )}
        ...
        {status === 'success' && (
          <div>
            ...
            <p className="text-sm text-gray-600 mt-2">Your account has been successfully verified.</p>
        ...
        {status === 'error' && (
          <div>
            ...
            <p className="text-xs text-gray-500 mb-4">You can try registering again or contact support if the problem persists.</p>
            <button
